    ...
    const fetchPosts = async () => {
      try {
        ...
        if (sort === "newest") {
          ...
          // ※複合インデックスが必要な場合あり
          snap = await getDocs(query(col, orderBy("highlight", "desc"), orderBy("createdAt", "desc")));
        } else if (sort === "title") {
          snap = await getDocs(query(col, orderBy("title", "asc")));
        } else {
          // sort === "category" は配列のため Firestore では並べ替え不可 → まず新着順で取得
          snap = await getDocs(query(col, orderBy("createdAt", "desc")));
        }

        const mapped: Post[] = snap.docs.map((d: QueryDocumentSnapshot<DocumentData>) => {
          const data = d.data();

          // createdAt の安全変換（Timestamp | undefined 両対応）
          ...
          const category: string[] = Array.isArray(data.category)
            ? data.category
        ...
        // カテゴリ順はクライアント側でソート（一次キー：先頭カテゴリ、二次キー：新着）
        if (sort === "category") {
          mapped.sort((a, b) => {
            const ac = (a.category?.[0] ?? "").toLowerCase();
            const bc = (b.category?.[0] ?? "").toLowerCase();
            if (ac !== bc) return ac.localeCompare(bc);
            const at = a.createdAt?.getTime() ?? 0;
            const bt = b.createdAt?.getTime() ?? 0;
            return bt - at; // 同カテゴリ内は新着優先
          });
        }
          ...
          >
            <h1
              style={{
                ...
                letterSpacing: 2,
                margin: 0,
                textShadow: "0 2px 10px #f700311a",
              }}
            >
              記事編集・一覧
            </h1>
            ...
            <button
              ...
              style={{
                background: selectedIds.length
                  ? "linear-gradient(90deg,#f70031 20%,#d4af37 90%)"
                  : "#eee",
                color: selectedIds.length ? "#fff" : "#bbb",
                border: "none",
                borderRadius: 8,
                fontWeight: 900,
                fontSize: 14,
                padding: "6px 15px",
                boxShadow: "0 2px 8px #f7003122",
                marginLeft: 5,
              ...
              <li
                ...
                style={{
                  ...
                  background: post.highlight
                    ? "linear-gradient(90deg,#fff7f2 60%,#fef2f2 100%)"
                    : "#fff",
                  boxShadow: "0 4px 16px #22336611",
                  ...
                  >
                    ...
                    {post.tags?.length ? (
                      <span style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                        ...
                        {post.tags.length > 3 && <span style={{ color: "#5b8dee" }}>…</span>}
                      </span>
                    ) : null}
                  </div>
                </div>
